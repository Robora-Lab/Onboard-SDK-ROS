  ...
  dji_sdk::srv::MissionWpUpload::Response::SharedPtr response)
{
  ...
  RCLCPP_INFO(this->get_logger(), "waypoint mission initialized and uploaded");
}
...
{
  RCLCPP_DEBUG(this->get_logger(), "called missionWpActionCallback");

  if (vehicle->missionManager->wpMissionVector.size() == 0)
  {
    RCLCPP_ERROR(this->get_logger(), "no waypoint mission uploaded");
    response->result = false;
  }
  ...
  dji_sdk::srv::MissionWpSetSpeed::Response::SharedPtr response)
{
  ...
  {
    RCLCPP_DEBUG(this->get_logger(), "wpSetSpeedCallback ack value: %d", (uint32_t)velAck.ack.data);
    response->result = false;
  }
  else
  {
  ...
  dji_sdk::srv::MissionHpAction::Response::SharedPtr response)
{
  ...
  {
    RCLCPP_ERROR(this->get_logger(), "no hotpoint mission uploaded");
    response->result = false;
  }
