                                    ...
                                    dji_sdk::srv::Activation::Response::SharedPtr response)
{
  ...
  else
  {
    ...
    RCLCPP_DEBUG(this->get_logger(), "drone activated");
  }
}
                                 ...
                                 dji_sdk::srv::CameraAction::Response::SharedPtr response)
{
  ...
  {
    RCLCPP_WARN(this->get_logger(), "unknown request task in cameraActionCallback");
    response->result = false;
  }
}
...
{
  RCLCPP_DEBUG(this->get_logger(), "called setHardsyncCallback");
  if (request->frequency == 0)
  {
    RCLCPP_INFO(this->get_logger(), "Call setSyncFreq with parameters (freq=%d, tag=%d). Will do one "
             "time trigger...",
  ...
  RCLCPP_INFO(this->get_logger(), "In valid frequency!");
  response->result = false;
}

void DJISDKNode::queryVersionCallback(const dji_sdk::srv::QueryDroneVersion::Request::SharedPtr request,
                                           ...
                                           dji_sdk::srv::Stereo240pSubscription::Response::SharedPtr response)
{
  ...
  if (request->unsubscribe_240p == 1)
  {
    ...
    response->result = true;
    RCLCPP_INFO(this->get_logger(), "unsubscribe stereo 240p images");
    return;
  }
  ...
  {
    response->result = false;
    RCLCPP_WARN(this->get_logger(), "Stereo 240p subscription service failed, please check your request content.");
  }
}
                                            ...
                                            dji_sdk::srv::StereoDepthSubscription::Response::SharedPtr response)
{
  ...
  {
    RCLCPP_WARN(this->get_logger(), "no depth image is subscribed");
    return;
  }
                                          ...
                                          dji_sdk::srv::StereoVGASubscription::Response::SharedPtr response)
{
  ...
  if (request->unsubscribe_vga == 1)
  {
    ...
    response->result = true;
    RCLCPP_INFO(this->get_logger(), "unsubscribe stereo vga images");
    return;
  }
  ...
  {
    RCLCPP_ERROR(this->get_logger(), "VGA subscription frequency is wrong");
    response->result = false;
    return;
  }
  ...
  {
    response->result = false;
    RCLCPP_WARN(this->get_logger(), "Stereo VGA subscription service failed, please check your request content.");
  }
}
...
{
  RCLCPP_DEBUG(this->get_logger(), "called cameraStreamCallback");
  void result = false;
